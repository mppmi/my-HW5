...
router.get("/", (req, res) => {
	const select = "select * from stars";
...
router.get("/:id", (req, res) => {
	...
	const select = "select * from stars where imdbID = ?";
...
router.post("/", (req, res) => {
    ...
    const insert = "INSERT INTO stars (imdbID,pid)"+
	"VALUES (?, ?)"
...
router.delete("/", (req, res) => {
	const imdbID = req.query.imdbID;
    const pid = req.query.pid;
	const sql = "delete from stars where imdbID = ? and pid = ?";
